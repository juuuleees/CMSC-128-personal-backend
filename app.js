...
const RestoRouter = require('./routes/store-router');
const AuthoRouter = require('./routes/autho-router');
...
app.get('/', (req, res) => {
  ...
  res.sendFile(path.join(__dirname + '/login.html')); // Absolute path of html file. Basically sends the intended html file to 'localhost:<port>/'
});

app.use('/eatwiseAutho', AuthoRouter); // Uses studentrouter and can be invoked by putting 'locahost:<port>/eatwise' on the address bar. Ofc /eatwise has nothing to return except routes lmao
